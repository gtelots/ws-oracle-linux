...
# Copy core packages installation script
...
RUN --mount=type=cache,target=/var/cache/dnf \
    ...
    chmod +x /opt/laragis/packages/pkg-core.sh && \
    TZ="${TZ}" PYTHON_VERSION="${PYTHON_VERSION}" \
    /opt/laragis/packages/pkg-core.sh
...
RUN ROOT_PASSWORD="${ROOT_PASSWORD}" \
