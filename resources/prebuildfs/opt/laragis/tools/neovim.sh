...
# Configuration
readonly TOOL_NAME="nvim"
readonly TOOL_VERSION="${NEOVIM_VERSION:-0.10.3}"
...
install_tool(){
  ...
  # Neovim uses different architecture naming
  case "$arch" in
    "amd64") arch="x86_64" ;;
    "arm64") arch="aarch64" ;;
  esac
  
  local download_url="https://github.com/neovim/neovim/releases/download/v${TOOL_VERSION}/nvim-${os}64.tar.gz"
  
  local temp_dir="$(mktemp -d)"
  local tar_file="${temp_dir}/nvim.tar.gz"
  ...
  cp -r "${temp_dir}/share/"* "/usr/local/share/" && \
  cp -r "${temp_dir}/lib/"* "/usr/local/lib/"

  # Create symlink for vim compatibility
  ln -sf "${INSTALL_DIR}/nvim" "${INSTALL_DIR}/vim"
