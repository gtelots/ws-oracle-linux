...
# eza
# =============================================================================
# DESCRIPTION: A modern, maintained replacement for 'ls' with colors and Git integration
...
# Configuration
readonly TOOL_NAME="eza"
readonly TOOL_VERSION="${EZA_VERSION:-0.20.11}"
...
install_tool(){
  ...
  # eza uses different architecture naming
  case "$arch" in
    "amd64") arch="x86_64" ;;
    "arm64") arch="aarch64" ;;
  esac

  local download_url="https://github.com/eza-community/eza/releases/download/v${TOOL_VERSION}/eza_${arch}-unknown-${os}-gnu.tar.gz"
  ...
  # Create compatibility symlink for exa users
  ln -sf "${INSTALL_DIR}/${TOOL_NAME}" "${INSTALL_DIR}/exa"
