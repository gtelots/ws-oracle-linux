...
# Configuration
readonly TOOL_NAME="rg"
readonly TOOL_VERSION="${RIPGREP_VERSION:-14.1.1}"
...
install_tool(){
  ...
  # ripgrep uses different architecture naming
  case "$arch" in
    "amd64") arch="x86_64" ;;
    "arm64") arch="aarch64" ;;
  esac
  
  local download_url="https://github.com/BurntSushi/ripgrep/releases/download/${TOOL_VERSION}/ripgrep-${TOOL_VERSION}-${arch}-unknown-${os}-musl.tar.gz"
