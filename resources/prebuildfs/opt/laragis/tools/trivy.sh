...
# Trivy
# =============================================================================
# DESCRIPTION: A comprehensive security scanner for vulnerabilities in containers
...
readonly TOOL_VERSION="${TRIVY_VERSION:-0.58.1}"
...
install_tool(){
  ...
  # Trivy uses different architecture naming
  case "$arch" in
    "amd64") arch="64bit" ;;
    "arm64") arch="ARM64" ;;
  esac
  
  # Capitalize OS name for Trivy
  case "$os" in
    "linux") os="Linux" ;;
    "darwin") os="macOS" ;;
  esac
  
  local download_url="https://github.com/aquasecurity/trivy/releases/download/v${TOOL_VERSION}/trivy_${TOOL_VERSION}_${os}-${arch}.tar.gz"
